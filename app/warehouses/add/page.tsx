...
export default function AddWarehousePage() {
  ...
  const generateWarehouseCode = () => {
    const codes = ["WH", "STORE", "DEPOT", "HUB", "CENTER"]
    const randomCode = codes[Math.floor(Math.random() * codes.length)]
    const randomNum = Math.floor(Math.random() * 999) + 1
    setFormData((prev) => ({
      ...prev,
      code: `${randomCode}-${String(randomNum).padStart(3, "0")}`,
    }))
  }
      ...
      <SidebarInset>
        ...
        <div className="flex flex-1 flex-col gap-4 p-4 pt-0">
            ...
            <div className="space-y-6">
              ...
              <Card>
                ...
                <CardContent className="space-y-2">
                  ...
                  <div className="flex justify-between text-sm">
                    <span>Location:</span>
                    <span>{formData.city && formData.state ? `${formData.city}, ${formData.state}` : "..."}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span>Manager:</span>
                    <span>
                      {formData.manager ? availableUsers.find((u) => u.id === formData.manager)?.name : "Not assigned"}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span>Capacity:</span>
                    <span>{formData.capacity ? `${Number(formData.capacity).toLocaleString()} sq ft` : "..."}</span>
                  </div>
                  <div className="flex justify-between text-sm">
          ...
          <div className="flex justify-end gap-4 mt-8">
            ...
            <Button
              disabled={
                !formData.code || !formData.name || !formData.address || !formData.city || selectedUsers.length === 0
              }
            >
              Create Warehouse
            </Button>
